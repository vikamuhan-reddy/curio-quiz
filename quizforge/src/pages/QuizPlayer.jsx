...
function JokeScreen({ joke, countdown }) {
  const [showPunchline, setShowPunchline] = useState(false);

  useEffect(() => {
    setShowPunchline(false);
    const t = setTimeout(() => setShowPunchline(true), 1800);
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 animate-scale-in"
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="card p-8 text-center mb-6"
          ...
          <div className={`transition-all duration-700 ${showPunchline ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
            style={{ transform: showPunchline ? 'translateY(0)' : 'translateY(16px)' }}>
            <div className="px-4 py-3 rounded-xl"
              style={{ background: 'var(--accent-bg)', border: '1px solid var(--accent-border)' }}>
              <p className="font-bold text-base" style={{ color: 'var(--accent)', fontFamily: 'Sora, sans-serif' }}>
                {joke.punchline}
              </p>
            </div>
          </div>
        ...
        <div className="flex items-center justify-center gap-2 text-sm" style={{ color: 'var(--text-3)' }}>
          <div className="w-2 h-2 rounded-full animate-bounce" style={{ background: 'var(--accent)' }} />
          <span>Next question in <span className="font-bold tabular-nums" style={{ fontFamily: 'DM Mono, monospace', color: 'var(--accent)' }}>{countdown}s</span></span>
        </div>
      </div>
    </div>
  );
...
function LoadingScreen({ vehicle }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6"
      style={{ background: 'var(--bg)' }}>
      <div className="w-full max-w-sm text-center">
        <p className="text-sm font-medium mb-8" style={{ color: 'var(--text-3)', fontFamily: 'Sora, sans-serif' }}>
...
export default function QuizPlayer() {
  ...
  const [betweenQuestions, setBetweenQuestions] = useState(null);
  const [score, setScore] = useState(0);
  const [isPaused, setIsPaused] = useState(false);

  // Feature: joke screen
  const [currentJoke, setCurrentJoke] = useState(null);
  const [jokeCountdown, setJokeCountdown] = useState(5);
  const [showJoke, setShowJoke] = useState(false);

  // Feature: loading vehicle
  ...
  const showJokeAndLoad = (onDone) => {
    ...
    jokeTimerRef.current = setInterval(() => {
      count--;
      setJokeCountdown(count);
      if (count <= 0) {
        clearInterval(jokeTimerRef.current);
        setShowJoke(false);
        // Then show loading vehicle briefly
  ...
  useEffect(() => {
    ...
    socket.on('quiz:between-questions', ({ leaderboard, questionIndex, total }) => {
      if (timerRef.current) clearInterval(timerRef.current);
      setBetweenQuestions({ leaderboard, questionIndex, total });
  ...
  // ─── Joke screen ────────────────────────────────────────
  if (showJoke && currentJoke) {
    return <JokeScreen joke={currentJoke} countdown={jokeCountdown} />;
  }

  // ─── Loading screen ─────────────────────────────────────
  if (showLoading) {
    return <LoadingScreen vehicle={currentVehicle} />;
  }

  // ─── Between questions ──────────────────────────────────
  if (betweenQuestions) {
    // Trigger joke → load sequence on mount
    return (
      <BetweenQuestionsScreen
        betweenQuestions={betweenQuestions}
        score={score}
        handleLeave={handleLeave}
        onShowJoke={showJokeAndLoad}
      />
    );
  }
...
// ─── Between Questions screen (triggers joke on mount) ─────
function BetweenQuestionsScreen({ betweenQuestions, score, handleLeave, onShowJoke }) {
  const [jokeTriggered, setJokeTriggered] = useState(false);

  useEffect(() => {
    if (!jokeTriggered) {
      setJokeTriggered(true);
      // Small delay so leaderboard shows briefly first
      const t = setTimeout(() => {
        onShowJoke(null);
      }, 1800);
      return () => clearTimeout(t);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 animate-scale-in"
      ...
      <div className="w-full max-w-sm flex justify-end mb-3">
        <button onClick={handleLeave} className="text-sm" style={{ color: 'var(--text-3)' }}>✕ Leave</button>
      </div>
      <p className="text-sm mb-1" style={{ color: 'var(--text-3)' }}>
        Question {betweenQuestions.questionIndex} of {betweenQuestions.total} complete
      </p>
      <h2 className="text-3xl font-black mb-1" style={{ fontFamily: 'Sora, sans-serif', color: 'var(--text)' }}>
        ⚡ Leaderboard
      </h2>
      <p className="font-bold mb-6 text-sm" style={{ color: 'var(--accent)' }}>Your score: {score} pts</p>

      <div className="w-full max-w-sm space-y-2.5 mb-8">
        {betweenQuestions.leaderboard.map((p, i) => (
          <div key={i} className="card p-3.5 flex items-center gap-3 animate-slide-up"
            ...
            <span className="flex-1 font-semibold text-sm" style={{ color: 'var(--text)' }}>{p.nickname}</span>
            <span className="font-bold text-sm" style={{ color: 'var(--accent)' }}>{p.total_score} pts</span>
          </div>
        ))}
      </div>

      <div className="flex items-center gap-2 text-sm animate-pulse" style={{ color: 'var(--text-3)' }}>
        <div className="w-2 h-2 rounded-full animate-bounce" style={{ background: 'var(--accent)' }} />
        A joke is coming...
      </div>
    </div>
